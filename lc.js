...
// --- NÂNG CẤP HỌC SÂU (DEEP LEARNING WEIGHTS) ---
function updatePatternPerformance(type, patternId, isCorrect) {
  ...
  // Tăng dung lượng ghi nhớ lịch sử cầu từ 20 lên 50 ván để học dài hạn
  ...
  // Tính toán trọng số kết hợp giữa dài hạn và ngắn hạn (Phong độ)
  if (stats.recentResults.length >= 10) {
    ...
    if (ultraRecentAcc > 0.7 && recentAccuracy > 0.6) {
      // Cả phong độ ngắn hạn và độ ổn định dài hạn đều tuyệt vời -> Bơm trọng số mạnh
      newWeight = Math.min(3.5, oldWeight * 1.15); 
    } else if (ultraRecentAcc < 0.4 && recentAccuracy > 0.5) {
      // Dài hạn ổn định nhưng đang bị lệch nhịp tạm thời -> Giảm nhẹ
      newWeight = Math.max(0.3, oldWeight * 0.95);
    } else if (ultraRecentAcc <= 0.4 && recentAccuracy <= 0.45) {
      // Đánh kém toàn diện -> Ép trọng số cực thấp
      newWeight = Math.max(0.1, oldWeight * 0.85); 
    } else if (ultraRecentAcc >= 0.7 && recentAccuracy < 0.5) {
      // Dài hạn kém nhưng đang vào guồng ăn thông -> Bắt đầu bơm trọng số
...
function recordPrediction(type, phien, prediction, confidence, patterns) {
  ...
  // Tăng trí nhớ tổng từ 500 lên 1000 ván
...
async function verifyPredictions(type, currentData) {
  ...
  for (const pred of learningData[type].predictions) {
    ...
    if (actualResult) {
      ...
      // Tăng bộ đệm accuracy tổng lên 100 để đo đạc chuẩn xác hơn
      ...
      if (pred.patterns && pred.patterns.length > 0) {
        pred.patterns.forEach(patternName => {
          // Lọc loại bỏ các tag VIP trước khi tìm patternId
...
// --- THUẬT TOÁN ĐẢO THÔNG MINH BẤT TỬ VIP (GIỮ NGUYÊN CẦU CỨNG) ---
function getSmartPredictionAdjustment(type, prediction, patterns) {
  const streakInfo = learningData[type].streakAnalysis;
  
  // 1. Nhận diện các cầu siêu cứng mà ta KHÔNG BAO GIỜ đảo bậy (1-1, bệt, rồng)
  let hasStrongUnbreakablePattern = false;
  
  if (patterns && patterns.length > 0) {
    const strongPatternTypes = ['cau_bet', 'cau_dao_11', 'cau_rong'];
    for (const p of patterns) {
      if (p.detected && p.confidence >= 65 && strongPatternTypes.includes(p.patternId)) {
        hasStrongUnbreakablePattern = true;
        break;
      }
    }
  }
  ...
  // Tool báo sai quá 2 tay (<= -2) -> Bật chế độ đảo liên tục cho tới khi thắng (streak về > 0)
  if (streakInfo.currentStreak <= -2) {
    if (hasStrongUnbreakablePattern) {
      // Nếu đang gặp Cầu 1-1 hoặc Cầu Bệt rõ ràng -> Tôn trọng cầu, đánh thuận không đảo
      return prediction; 
    } else {
      // Nếu cầu linh tinh -> Kích hoạt đảo kết quả
      return prediction === 'Tài' ? 'Xỉu' : 'Tài';
    }
  }
  
  // 3. Logic Machine Learning gốc kết hợp dài hạn
  ...
  patterns.forEach(p => {
    ...
    if (patternId) {
      ...
      if (stats && stats.recentResults.length >= 5) {
        ...
        // Pha trộn 70% ngắn hạn và 30% dài hạn để thuật toán ra quyết định mượt mà
        const combinedAcc = (recentAcc * 0.7) + (longTermAcc * 0.3);
...
// ==================== CÁC HÀM PHÂN TÍCH GIỮ NGUYÊN 100% GỐC ====================
...
// ==================== HÀM TÍNH TOÁN DỰ ĐOÁN CHÍNH (ĐƯỢC GẮN AI VIP) ====================
...
function calculateAdvancedPrediction(data, type) {
  ...
  // Dựa vào phân tích ban đầu, đưa ra kết quả gốc (chưa qua màng lọc VIP)
  let basePrediction = taiScore >= xiuScore ? 'Tài' : 'Xỉu';
  
  // ĐƯA VÀO BỘ LỌC ĐẢO CHIỀU VIP THÔNG MINH
  let finalPrediction = getSmartPredictionAdjustment(type, basePrediction, allPatterns);
  ...
  // Xử lý logs để user biết tool đang làm gì
  const streakInfo = learningData[type].streakAnalysis;
  if (finalPrediction !== basePrediction) {
    // Tool đã đảo chiều kết quả thành công do phát hiện chuỗi gãy liên tiếp
    factors.unshift(`[VIP ĐẢO CHIỀU CHỐNG GÃY] Đảo ${basePrediction} → ${finalPrediction} (Đang gãy ${Math.abs(streakInfo.currentStreak)} tay)`);
    baseConfidence += 8; // Tự tin hơn vì đang đi theo bài Đảo thông minh
  } else if (streakInfo.currentStreak <= -2) {
    // Đang gãy nhưng KHÔNG đảo, lý do là gặp 1-1 / bệt cứng
    const isStrong = allPatterns.some(p => p.detected && p.confidence >= 65 && ['cau_bet', 'cau_dao_11', 'cau_rong'].includes(p.patternId));
    if (isStrong) {
      factors.unshift(`[ÔM CẦU CỨNG VIP] Bỏ qua đảo chống gãy do gặp cầu quá đẹp`);
    }
  }
  ...
  finalConfidence = Math.max(60, Math.min(95, finalConfidence));
...
app.listen(PORT, '0.0.0.0', () => {
  ...
  console.log('Lau Cua 79 - Advanced Tai Xiu Prediction API v7.0 VIP AI');
  console.log('');
  console.log('CẢI TIẾN VIP MỚI NHẤT:');
  console.log('  - THUẬT TOÁN ĐẢO BẤT TỬ (Chống gãy liên tục cho tới khi ăn)');
  console.log('  - NGOẠI LỆ CẦU CỨNG (Phát hiện bệt, 1-1, rồng tự động nhả Đảo để ôm cầu)');
  console.log('  - NÂNG CẤP HỌC SÂU (Deep Learning nhớ 1000 ván, kết hợp chuẩn 10-50 tay)');
  console.log('  - GIỮ NGUYÊN 100% Thuật toán phát hiện pattern gốc');
  console.log('');
